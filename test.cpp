...
public:
    ...
    m_color(color)
  {
	...
	auto get_node_size(double parent_size)
	{
		return 0.5 * parent_size;
	}
	...
	auto get_origin(QuadTree* parent, int n)
	{
		...
		return std::make_tuple<double, double>(
			...
			parent->m_y + 0.5 * get_node_size(parent->m_size) * std::get<1>(offset)
			);
	}

	bool need_split(double x, double y, double ox, double oy, double L, double k)
	{
		if (m_depth > 0)
		{
			auto d = std::max(
				std::min(std::abs(x - ox), std::abs(x - ox - L)),
	...
	QuadTree::Ptr make_child(int i)
	{
		...
		return std::make_shared<QuadTree>(m_depth - 1, 0.5 * m_size, quad.ox, quad.oy, quad.color);
	}
  ...
  void draw_recursive(IRender* render, double ox, double oy, int level, bool is_last)
  {
		using namespace std;
    ...
    else
    {
      ...
      for (int i = 0; i < 4; i++)
      {
        ...
        auto origin = get_origin(this, i);
        m_children[i]->draw_recursive(render, std::get<0>(origin), std::get<0>(origin), level + 1, i == 3);
      }
			if (!is_last)
				cout << string(level, ' ') << "]," << endl;
			else
